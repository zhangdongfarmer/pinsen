...
class YaochangModel extends Model {

  public function yaoChangList($args, $order, $field = '*', $page = 1, $pageSize = 10){
      $where['delete'] = 0;
  ...
  public function yaoChangOne( $args ){
      if( $args['id'] ) $where['id'] = $args['id'];
  ...
  public function updYaoChang($id, $args){
      if( empty( $id ) ){
          $data = [
      	        'state' => 0,
      	        'msg' => '参数错误'
  ...
  public function addYaoChang( $args ){
      if($args['name']) $row['name'] = $args['name'];
      if($args['province_id']) $row['province_id'] = $args['province_id'];
      if($args['city_id']) $row['city_id'] = $args['city_id'];
      if($args['area_id']) $row['area_id'] = $args['area_id'];
