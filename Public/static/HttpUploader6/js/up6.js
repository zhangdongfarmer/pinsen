...
function HttpUploaderMgr()
{
	...
	this.Config = {
		...
		//文件夹操作相关
		, "UrlFdCreate"		: "http://www.pinsenqihang.com/Public/Admin/uploader/db/fd_create.php"
		, "UrlFdComplete"	: "http://www.pinsenqihang.com/Public/Admin/uploader/db/fd_complete.php"
		, "UrlFdDel"	    : "http://www.pinsenqihang.com/Public/Admin/uploader/db/fd_del.php"
		//文件操作相关
		, "UrlCreate"		: "http://www.pinsenqihang.com/Public/Admin/uploader/db/f_create.php"
		, "UrlPost"			: "http://www.pinsenqihang.com/Public/Admin/uploader/db/f_post.php"
		, "UrlComplete"		: "http://www.pinsenqihang.com/Public/Admin/uploader/db/f_complete.php"
		, "UrlList"			: "http://www.pinsenqihang.com/Public/Admin/uploader/db/f_list.php"
		, "UrlDel"			: "http://www.pinsenqihang.com/Public/Admin/uploader/db/f_del.php"
        //x86
	...
	_this.UploadQueue 		= new Array();	//未上传队列
	_this.arrIdsErrStop 	= new Array(); 	//未上传项ID列表
	_this.arrFiles 			= new Array(); 	//上传的队列（ID列表）
	...
	this.FileListMgr =
	{
		...
		, "LoadTo": function (dom)//加载到指定控件
		{
		    ...
		    this.FileLister     = dom.find('div[name="files"]');
		    this.FileItemTemp   = dom.find('ul[name="tmpFile"]');
		    this.LoadData();
		}
		, "Init": function ()//初始化（已弃用）
		{
		...
		{
		    var ref = this;
		    var idLoc = _this.UploaderListCount++//为本地索引，idSvr文件和文件夹有冲突
		    this.FileItems[idLoc] = item;
			...
			else
			{
			    ...
			    liOp.find('span[name="btnReUp"]').click(function () { ref.ResumerFile(idLoc); });
			    liOp.find('span[name="btnDel"]').click(function () { ref.RemoveFile(idLoc); });
			}
			//添加到文件列表项集合
			this.FileItemCtls[idLoc] = ctl;
		...
		, "UploadComplete": function (idLoc, uper)//上传完成，将操作改为删除。
		{
			//文件已存在
		    if (typeof (this.FileItems[idLoc]) != "undefined")
		    {
		        var ref = this;
		        var ctl = this.FileItemCtls[idLoc];
		        var liPer = ctl.find('li[name="fper"]');
		        var liOp = ctl.find('li[name="fop"]');
		...
		, "ResumerFile": function (idLoc)//续传文件
		{
		    var item = this.FileItems[idLoc];
			//文件夹任务
			if (item.f_fdTask=="1")
			{
			...
			{
				var path = decodeURIComponent(item.pathLoc);
				_this.ResumeFile(path, item.PostedLength, item.PostedPercent, item.FileMD5, item.idSvr,item.pathSvr);
			}
			_this.OpenPnlUpload(); //打开上传面板
			this.RemoveFileCache(idLoc); //从内存中删除
			this.UploaderMgr.PostFirst();
		}
		, "RemoveFile": function (idLoc)//删除文件
		{
			var ref = this;
			var item = this.FileItems[idLoc];
			var ctl = this.FileItemCtls[idLoc];
			...
			$.ajax({
				...
				, data: { uid: item.uid, fid: item.idSvr, time: new Date().getTime() }
				, success: function (msg) {
					//删除成功
					if (msg == 1) {
						//删除HTML控件
		...
		, "RemoveFileCache": function (idLoc) {
		    this.FileItems[idLoc] = null;
		    this.FileItemCtls[idLoc].empty();
		    this.FileItemCtls[idLoc] = null;
		}
	};
	//初始化路径
	...
	{
	    //npapi
	    var com = '<embed name="ffPart" type="' + this.Config["XpiType"] + '" pluginspage="' + this.Config["XpiPath"] + '" width="1" height="1"/>';
	    //acx += '<div style="display:none">';
	    //文件上传控件
	    com += '<object name="ieUper" classid="clsid:' + this.Config["ClsidUploader"] + '"';
	    com += ' codebase="' + this.Config["CabPath"] + '#version=' + this.Config["Version"] + '" width="1" height="1" ></object>';
	    //文件夹选择控件
	    com += '<object name="iePart" classid="clsid:' + this.Config["ClsidPartition"] + '"';
	    com += ' codebase="' + this.Config["CabPath"] + '#version=' + this.Config["Version"] + '" width="1" height="1" ></object>';
		...
		var html = '<div class="combinBox">\
						...
						<div class="cbBody" id="cbBody">\
							<ul name="cbItem" class="block"><li name="filesLoc" id="liUploadPanel"></li></ul>\
							<ul name="cbItem" class="cbItem"><li name="filesSvr" id="liListerPanel"></li></ul>\
						</div>\
					</div>';
		return com + html;
	};
	...
	this.BrowserChr = {
		...
		, "Setup": function ()//安装插件
		{
			document.write('<iframe style="display:none;" src="../HttpUploader/' + _this.Config["CrxPath"] + '"></iframe>');
		}
		, "OpenFileDialog": function ()//打开文件选择窗口
		{
			var obj = this.partition;
	...
	{
	    $("#pnlHeader").html('<a href="../HttpUploader/' + _this.Config["ExePath"] + '" target="_blank">请先安装控件</a>');
	};
	...
	this.NeedUpdate = function () {
	    if (this.Browser.Check())
	    {
	        ...
	        {
	            _this.pnlHeader.append('<br/><a href="../HttpUploader/' + _this.Config["ExePath"] + '" target="_blank">控件已更新，请下载最新版</a>');
	        }
	    }
	};
	...
	this.Init = function (dom)
	{
	    ...
	    _this.ffPart = dom.find('embed[name="ffPart"]').get(0);
	    _this.iePart = dom.find('object[name="iePart"]').get(0);
	    _this.ieUper = dom.find('object[name="ieUper"]').get(0);

	    var panel           = filesLoc.html(_this.GetHtmlFiles());
	    ...
	    _this.Droper        = panel.find('object[name="ieDroper"]').get(0);
	    ...
	    //drag files
	    if (null != _this.Droper) _this.Droper.OnDroped = function ()
	    {
